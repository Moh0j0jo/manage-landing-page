...
const Testimonials = () => {
  ...
  return (
      ...
      <section className="flex flex-col justify-center items-center mb-32 text-center">
        <h2 className="mb-12 text-3xl font-bold">What they've said</h2>

        <div className="relative">
          {slides.map((slide, index) => (
            <div
              key={slide.id}
              className={`relative w-full overflow-hidden ${index === currentSlide ? "block" : "hidden"}`}
              style={{ backfaceVisibility: "hidden" }}
            >
              <img className="mx-auto mb-6 rounded-full shadow-lg dark:shadow-black/20 w-[150px]"
                src={slide.avatar} alt="avatar" />
              <div className="flex flex-wrap justify-center">
                <div className="w-full shrink-0 grow-0 basis-auto px-3 lg:w-8/12">
                  <h5 className="mb-2 text-lg font-bold">{slide.name}</h5>
          ...
          <button
            className="absolute top-0 bottom-0 left-0 z-[1] flex w-[15%] items-center justify-cente"
            type="button"
        ...
        <span
          ...
          <button
            className="absolute top-0 bottom-0 right-0 z-[1] flex w-[15%] items-center justify-cente"
            type="button"
        ...
        <Button label={"Get Started"} styles='w-40 h-9 bg-bright-red text-white' />
      </section>
