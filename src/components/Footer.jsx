...
const Footer = () => {
  ...
  const handleEmailChange = (event) => {
    ...
    setEmail(value);
    if (value === "") {
      // Assume no validation message if the field is empty
      setIsValid(true);
    } else {
      // Apply regex check only if the field is not empty
  ...
  return (
    <footer className="flex flex-col h-svh md:h-fit">
      ...
      >
        ...
        <Button
          label={"Get Started"}
          styles="w-44 h-9 bg-white text-bright-red"
        />
      </div>

      <div
        className="h-3/5 flex flex-col gap-9 bg-very-dark-blue p-10 text-center
      ...
      >
        ...
        <div className="order-3 w-full h-10 flex flex-row justify-center items-center gap-7  md:order-6 md:row-span-1 md:col-span-1 md:justify-start">
          <span className="w-6 h-full flex justify-center items-center cursor-pointer">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="#FFF"
              viewBox="0 0 20 20"
              className="w-full h-full hover:fill-bright-red"
            >
        ...
        <div className="order-1 w-full flex items-center justify-center gap-3 relative  md:order-2 md:row-span-1 md:col-span-1 md:gap-1">
          <input
            className="w-2/3 h-12 rounded-3xl py-3 px-7 invalid:border-bright-red valid:border-green-500 font-beVietnam md:text-xs md:w-66 md:px-5"
            type="email"
            value={email}
          ...
          />
          {!isValid && (
            <div className="absolute -bottom-6 left-9 text-sm italic text-bright-red md:left-0 lg:left-12">
